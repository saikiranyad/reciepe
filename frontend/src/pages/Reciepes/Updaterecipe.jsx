...
//     images: [],
//     collabortors: '',
//     steps: '' // Steps as newline-separated string
//   });
...
//         const ids = formData.collabortors.split(',').map(id => id.trim());
//         ids.forEach(id => recipeData.append('collabortors', id));
//       }

//       // Append steps
//       const stepList = formData.steps.split('\n').map(step => step.trim()).filter(step => step !== '');
//       stepList.forEach(step => recipeData.append('steps', step));
...
//               <input type="text" name="title" value={formData.title} onChange={handleChange} required className="w-full border border-gray-300 rounded px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-rose-400" />
//             </div>

//             <div>
//               <label className="block text-sm font-medium text-rose-600 mb-1">Description</label>
//               <textarea name="description" value={formData.description} onChange={handleChange} required className="w-full border border-gray-300 rounded px-3 py-2 text-base resize-none h-24 focus:outline-none focus:ring-2 focus:ring-rose-400" />
//             </div>

//             <div>
//               <label className="block text-sm font-medium text-rose-600 mb-1">Ingredients (comma-separated)</label>
//               <textarea name="ingreidients" value={formData.ingreidients} onChange={handleChange} required className="w-full border border-gray-300 rounded px-3 py-2 text-base resize-none h-20 focus:outline-none focus:ring-2 focus:ring-rose-400" />
//             </div>

//             <div>
//               <label className="block text-sm font-medium text-rose-600 mb-1">Steps (one per line)</label>
//               <textarea name="steps" value={formData.steps} onChange={handleChange} required className="w-full border border-gray-300 rounded px-3 py-2 text-base resize-none h-32 focus:outline-none focus:ring-2 focus:ring-rose-400" />
//             </div>

//             <div>
//               <label className="block text-sm font-medium text-rose-600 mb-1">Upload Images</label>
//               <input type="file" name="images" onChange={handleFileChange} multiple required className="block w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:border file:rounded file:border-rose-500 file:text-rose-500 hover:file:bg-rose-50" />
//             </div>

//             <div>
//               <label className="block text-sm font-medium text-rose-600 mb-1">Collaborator IDs (comma-separated)</label>
//               <input type="text" name="collabortors" value={formData.collabortors} onChange={handleChange} className="w-full border border-gray-300 rounded px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-rose-400" />
//             </div>

//             <button type="submit" className="w-full bg-rose-500 hover:bg-rose-600 text-white font-semibold py-2 rounded transition duration-200">
//               Submit
//             </button>
//           </form>
...
//           <div className="mt-6">
//             <h3 className="text-sm font-medium text-rose-600 mb-2">Uploaded Images</h3>
//             <div className="grid grid-cols-3 gap-2">
//               {formData.images.length > 0 ? (
//                 Array.from(formData.images).map((image, index) => (
//                   <div key={index} className="w-full h-32 bg-gray-100 rounded overflow-hidden">
//                     <img src={image.imageurl} alt={`uploaded-${index}`} className="w-full h-full object-cover" />
//                   </div>
//                 ))
//               ) : (
...
//   const handleFileChange = (e) => {
//     setFormData(prev => ({ ...prev, images: [...prev.images, ...e.target.files] }));
//   };

//   const handleImageDelete = (index) => {
//     const updatedImages = [...formData.images];
//     updatedImages.splice(index, 1); // Remove image at the specified index
...
//             <div>
//               <label className="block text-sm font-medium text-rose-600 mb-1">Upload New Images (optional)</label>
//               <input type="file" name="images" onChange={handleFileChange} multiple className="block w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:border file:rounded file:border-rose-500 file:text-rose-500 hover:file:bg-rose-50" />
//             </div>
...
//               {formData.images.length > 0 ? (
//                 Array.from(formData.images).map((image, index) => (
//                   <div key={index} className="w-full h-32 bg-gray-100 rounded overflow-hidden relative">
//                     <img src={image.imageurl || URL.createObjectURL(image)} alt={`uploaded-${index}`} className="w-full h-full object-cover" />
//                     <button
